...
def parse_report(
    ...
    conn.execute(
        """UPDATE reports SET
           location_text = ?, lat = ?, lng = ?, incident_type = ?,
           people_mentioned = ?, has_children = ?, language = ?, processed = 1
           WHERE id = ?""",
...
async def run_field_report_agent() -> dict[str, Any]:
    ...
    conn = get_connection()
    count = conn.execute("SELECT COUNT(*) FROM reports WHERE processed = 1").fetchone()[0]
    conn.close()
